...
## M7 — Trash, Settings, app lock (FR-010, 013, 014) ✅ (2026-07-24)
**Trash:** "Move to Trash" ghost affordance in the read view (from Life and Remember, both refresh after), `TrashView` overlay (⌘⇧T / app menu "Show Trash"): restore per row, Empty Trash behind a confirmation dialog → hard delete + `MediaStore.remove` of returned file paths (AC-008; row lifecycle covered by M2 tests). **Settings (⌘,):** design-styled window — AI on/off, OpenRouter key via `SecureField` → **`KeychainStore`** (Keychain Services; settings table records only that a key exists), editable per-stage model ids (defaults per §3.4), embeddings pinned `bge-m3·1024`, STT model radio group, app-lock toggle. **App lock (FR-014):** `AppLock` + `LockView` gate the whole shell via `LAContext` (`.deviceOwnerAuthentication` — Touch ID with password fallback; fails open if no auth is available so the user is never locked out); no content renders behind the lock.
**Result:** app builds/launches; DB greps clean of key material (structurally guaranteed — the key never passes through the settings repo). Touch ID prompt itself = human check.

## M8 — Voice capture (FR-029 brought forward; on-device) ✅ (2026-07-24)
